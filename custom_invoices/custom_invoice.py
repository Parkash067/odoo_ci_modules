...
class dummy_invoice(osv.osv):
    ...
    _columns = {
        ...
        'invoice_line': fields.one2many('dummy.invoice.line','name','Invoice Lines',store=True),
...
class dummy_invoice_lines(osv.osv):
    _name = "dummy.invoice.line"
    _rec_name = 'products'
    _columns = {
        'name': fields.many2one('dummy.invoice','Name', store=True),
